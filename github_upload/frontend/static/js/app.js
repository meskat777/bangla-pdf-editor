...
const AppState = {
    ...
    undoStack: [],
    redoStack: [],
    pdfMetadata: null  // Store PDF metadata for zoom compensation
};

/**
 * GEMINI SOLUTION: Calculate Zoom-Compensated Font Size
 * Converts PDF Points to Browser Pixels accounting for zoom and device pixel ratio
 * 
 * @param {number} pdfFontSize - Original font size from PDF in points
 * @param {number} canvasWidth - Current canvas width in pixels
 * @param {number} originalPdfWidth - Original PDF page width in points
 * @returns {number} - Compensated font size in pixels for display
 */
function calculateZoomCompensatedSize(pdfFontSize, canvasWidth, originalPdfWidth) {
    if (!canvasWidth || !originalPdfWidth || !pdfFontSize) {
        return pdfFontSize; // Fallback to original
    }
    
    // 1. Calculate scale factor (how much the PDF is scaled in browser)
    const scaleFactor = canvasWidth / originalPdfWidth;
    
    // 2. Device pixel ratio (for high-DPI/retina displays)
    const dpr = window.devicePixelRatio || 1;
    
    // 3. Convert PDF points to browser pixels
    // Standard: 72 points = 1 inch, browser: 96 pixels = 1 inch
...
/**
 * Convert browser pixels back to PDF points for sending to backend
 * @param {number} pixelSize - Font size in pixels
 * @param {number} canvasWidth - Current canvas width in pixels
 * @param {number} originalPdfWidth - Original PDF page width in points
 * @returns {number} - Font size in PDF points
 */
function pixelsToPdfPoints(pixelSize, canvasWidth, originalPdfWidth) {
    if (!canvasWidth || !originalPdfWidth || !pixelSize) {
        return pixelSize; // Fallback
    }
    
    const scaleFactor = canvasWidth / originalPdfWidth;
    const pdfPoints = pixelSize / (scaleFactor * (96 / 72));
    
    return Math.round(pdfPoints * 10) / 10; // Round to 1 decimal
}
...
// Close PDF and reset to upload screen
function closePDF() {
    // Confirm before closing if there are modifications
    if (AppState.modifications.length > 0) {
        const confirmed = confirm('আপনার পরিবর্তনগুলি সংরক্ষিত না হতে পারে। বন্ধ করতে চান? | You may have unsaved changes. Do you want to close?');
        if (!confirmed) return;
    }
    ...
    // Update status
    updateStatus('পিডিএফ বন্ধ করা হয়েছে | PDF closed');
    updateSessionInfo();
    updatePageInfo();
...
async function loadPDFPage(pageNumber) {
    ...
    try {
        ...
        if (result.success) {
            // Store PDF metadata for zoom compensation
            if (!AppState.pdfMetadata && AppState.pdfData) {
...
function openEditPanel(textBlock) {
    ...
    // Show the visual size (in pixels) that matches what user sees on screen
    let displayFontSize = Math.round(textBlock.size);
    let originalPdfSize = textBlock.size;
    ...
    if (AppState.pdfMetadata && elements.pdfCanvas) {
        ...
        // Store original PDF font size for reference
        elements.fontSize.dataset.originalSize = textBlock.size;
        ...
        if (fontSizeInfo) {
            const scaleFactor = elements.pdfCanvas.width / AppState.pdfMetadata.width;
            fontSizeInfo.innerHTML = `
                📏 <strong>PDF:</strong> ${originalPdfSize.toFixed(1)}pt | 
                <strong>Display:</strong> ${displayFontSize}px | 
                <strong>Zoom:</strong> ${(scaleFactor * 100).toFixed(0)}%
            `;
            fontSizeInfo.style.color = '#667eea';
        }
    } else {
        // No compensation available
        const fontSizeInfo = document.getElementById('fontSizeInfo');
        if (fontSizeInfo) {
            fontSizeInfo.innerHTML = `📏 <strong>Size:</strong> ${originalPdfSize}pt`;
            fontSizeInfo.style.color = '#666';
        }
    }
    
    elements.fontSize.value = displayFontSize;
    ...
    try {
        const response = await fetch(`${API_BASE}/api/text/edit`, {
            ...
            body: JSON.stringify({
                ...
                original_text: AppState.selectedTextBox.text,
                new_text: newText,
                font: font,
                font_size: fontSize,
                color: color,
